...
ep_rewards = {'eventbased': [], 'locations': [], 'mix': []}
ep_won = {'eventbased': [], 'locations': [], 'mix': []}
...
state_representations = ['eventbased', 'locations', 'mix']

for i, state_representation in enumerate(state_representations):

    np.random.seed(10)

    q = qlearning.QLearning(LEARNING_RATE, DISCOUNT, ACTION_SPACE_SIZE)
    ...
    for episode in range(EPISODES):
        ...
        if player_is_winner:
            ep_won[state_representation].append(100)
        else:
            ep_won[state_representation].append(0)

        if END_EPSILON_DECAYING >= episode >= START_EPSILON_DECAYING:
            epsilon -= epsilon_decay_value

        ep_rewards[state_representation].append(episode_reward)
